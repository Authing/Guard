...
AuthingGuard.prototype = {
  ...
  //   const state =
  //     this.querySearch("state") || Math.ceil(Math.random() * Math.pow(10, 6));
  //   const appId =
  //     this.querySearch("app_id") || this.querySearch("client_id") || "";
  ...
  //   const signature = this.querySearch("Signature") || "";
  //   const signAlg = this.querySearch("SignAlg") || "";
  //   const scope = this.querySearch("scope") || "";
  //   this.SAMLIdPURL = `${ssoHost}/login/authorize/confirm?app_id=${appId}&context=SAMLIdP&bindings=${bindings}&SAMLRequest=${SAMLRequest}&Signature=${signature}&SignAlg=${signAlg}`;
  ...
  //         location.href =
  //           url || `${location.origin}/login?app_id=${appInfo._id}`;
  //       }
  //     } else {
  ...
  //         secondLvlDomain.domain +
  //         `") {   
  //               _id,
  //               name,
  //             }
  //           }`;
  //       oAuthGql.request({ query }).then(e => {
  //         let appInfo;
  //         switch (context) {
  //           case "OIDC":
  ...
  //               } else {
  //                 const url = `${location.origin}/oauth/oidc/auth?client_id=${
  //                   qInfo._id
  ...
  //   if (!uuid) {
  //     location.href =
  //       location.pathname + "error?message=缺少 OIDC 所必须的参数 uuid";
  //   }
