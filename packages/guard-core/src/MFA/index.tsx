...
export const GuardMFAView: React.FC = () => {
  ...
  const __codePaser = (code: number, msg?: string) => {
    ...
    if (action?.action === 'changeModule') {
      const m = action.module ? action.module : GuardModuleType.ERROR
      const init = action.initData ? action.initData : {}
  ...
  const renderBack = useMemo(() => {
    ...
    return <BackLogin />
  }, [currentMethod, initData.applicationMfa, mfaBackState])
